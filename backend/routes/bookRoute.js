module.exports = function(app, Book)
{
    ...
    app.get('/api/books/author/:author', function(req, res){
        Book.find({author: req.params.author}, {_id: 0, title: 1, published_date: 1},  function(err, books){
            if(err) return res.status(500).json({err: err});
            if(books.length === 0) return res.status(404).json({err: 'book not found'});
            res.json(books);
        })
    });
    ...
    app.put('/api/books/:book_id', function(req, res){
        Book.update({ _id: req.params.book_id }, { $set: req.body }, function(err, output){
            if(err) res.status(500).json({ err: 'database failure' });
            console.log(output);
            if(!output.n) return res.status(404).json({ err: 'book not found' });
    ...
    */
    ...
    app.delete('/api/books/:book_id', function(req, res){
        Book.remove({ _id: req.params.book_id }, function(err, output){
            ...
            if(!output.result.n) return res.status(404).json({ err: "book not found" });
